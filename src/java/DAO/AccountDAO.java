...
public class AccountDAO extends DAO {
    ...
    public void updateAccountID(Account account) {
        try {
            String sql = "UPDATE Account\n"
                    + "SET  account_name = ?,[password] = ?,customer_name = ?,email=?,role_id = ?\n"
                    + "WHERE account_id = ?";
            PreparedStatement stm = con.prepareStatement(sql);
            stm.setString(1, account.getAccountName());
            stm.setString(2,account.getPassword());
            stm.setNString(3, account.getCustomer_name());
            stm.setString(4, account.getEmail());
            stm.setInt(5, account.getRoleId());
    ...
    public static void main(String[] args) {
        ...
        System.out.println(dao.getOneAcc(3));
    }
}
